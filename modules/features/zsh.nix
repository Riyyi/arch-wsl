      ...
      config = {
        ...
        home.packages =
          ...
          ++ lib.optionals (config.preferences.distro == "ubuntu") [
            fzf # Ubuntu 24.04 ships old fzf 0.44.1, which doesnt have the --zsh flag
            zsh-history-substring-search
          ];
        ...
        programs.zsh = {
          ...
          shellAliases = {
            ...
            gds = "git diff --staged";
            gf = "git fetch";
            gl = "git log --graph --abbrev-commit --decorate --format=format:'%C(bold blue)%h%C(reset) - %C(bold cyan)%ai%C(reset) %C(bold green)(%ar)%C(reset)%C(bold yellow)%d    %C(reset)%n''          %C(white)%s%C(reset) %C(dim white)- %an <%cn>%C(reset)'";
            gla = "gl --all";
